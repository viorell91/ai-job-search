...
describe("CLI contract", () => {
  test("no command prints help and exits 1", async () => {
    ...
    expect(r.stdout).toContain("startupjobs-cli")
  })
  ...
  test("non-numeric --limit is rejected before any request", async () => {
    const r = await runCLI(["search", "-q", "x", "--limit", "abc"])
    expect(r.exitCode).toBe(1)
    expect(JSON.parse(r.stderr)).toMatchObject({ code: "BAD_ARG" })
  })

  test("non-numeric --scan-pages is rejected", async () => {
    const r = await runCLI(["search", "-q", "x", "--scan-pages", "week"])
    expect(r.exitCode).toBe(1)
    expect(JSON.parse(r.stderr)).toMatchObject({ code: "BAD_ARG" })
  })
