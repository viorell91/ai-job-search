...
describe("CLI contract", () => {
  test("no command prints help and exits 1", async () => {
    ...
    expect(r.stdout).toContain("arbeitsagentur-cli")
  })
  ...
  // Note: a bare token like "not-an-id" is NOT invalid here — Arbeitsagentur
  // reference numbers are alphanumeric with dashes, so that shape is accepted
  // and resolved against the API. Only input that cannot be a refnr is rejected.
  test("detail with an unparseable id errors without fetching", async () => {
    const r = await runCLI(["detail", "https://example.com/jobs"])
    expect(r.exitCode).toBe(1)
